...
fn request_and_persist_stage_ai_content(
    ...
    // Try Python AI worker first, fallback to direct DeepSeek
    if worker::worker_available() {
        logger::info("stage_ai: routing through AI Worker (LangGraph)");
        return request_via_worker(store, run_id, project_id, project_name, stage, defaults, feasibility);
    }

    logger::info("stage_ai: AI Worker unavailable, falling back to direct DeepSeek");
    request_direct_deepseek(store, run_id, project_id, project_name, stage, defaults, feasibility)
}
...
fn request_via_worker(
    ...
    insert_stage_event(
        ...
        &format!("已为 {} 阶段创建 LangGraph Agent。", stage_prompt_label(stage)),
    )?;
...
/// Direct DeepSeek fallback — original two-stage chain logic.
fn request_direct_deepseek(
    store: &Store,
    run_id: &str,
    project_id: &str,
    project_name: &str,
